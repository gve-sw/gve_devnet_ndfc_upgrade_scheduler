...
class NDFC:
    ...
    def setImagePolicy(self):
        ...
        Note: This call allows us to tag the new policy, then automatically
              stage & validate the image. Even if device is already assigned
              to the policy, this call will still stage & validate the image.
        """
        log.info(f"Setting device image policy to {config['upgrade']['policy']}...")

        # Build policy payload
        # Note: "stageValidate" is True, so this will push image to device and run
        #       validation checks
        policy_body = {"mappingList": [], "stageValidate": True}
    ...
    def checkReadyForUpgrade(self):
        """
        Query NDFC for device status & check to see that staging / validation
        has completed or not

        Once complete, sets self.ready_for_upgrade to True
        """
        ...
        for device in self.device_info:
            if device["imageStaged"] == "Success" and device["validated"] == "Success":
                ready_devices += 1
                continue
        ...
        if len(error_devices) > 0:
            log.error(
                f"[red]{len(error_devices)} devices hit an error and will be removed from the upgrade list:"
            )
            log.error(f"[red]{error_devices}")

        # Log / check if devices ready
        if ready_devices == total_devices:
            self.ready_for_upgrade = True
...
def startUpgrade() -> None:
    ...
    if config["upgrade"]["stage-image"]:
        ...
        log.info(
            "Waiting for devices to be ready for upgrade... (This may take a while)"
        )
        log.info(
